...
StateEstimator::StateEstimator(float gyro_noise_std, float gyro_bias_noise_std, float acc_noise_std, float acc_bias_noise_std, float gravity_noise_std)
    : current_state(), g_initialized(false), max_iteration(10), min_correspondence_threshold(20), huber_loss_delta(0.5), converge_threshold(0.01), lidar_noise_std(0.05) {
...
void StateEstimator::propagateIMU(const IMUData& imu_data) {
    ...
    StateWithStamp state_stamp = StateWithStamp(imu_data.timestamp, omega, acc,
                                                current_state.rotation, current_state.position, current_state.velocity, current_state.gravity, Sophus::SE3f(R_new, new_position));
...
void StateEstimator::undistortPointcloud(std::vector<PointCloud>& points) {
    ...
    end_it --;
    //spdlog::info("The upperbound tstamp {}, last pointcloud tstamp {}, {}, {}, {}", end_it->timestamp, points.back().timestamp, tmp_preint_list.front().timestamp, tmp_preint_list.back().timestamp, tmp_preint_list.size());
...
State StateEstimator::updateState(std::vector<Eigen::Vector3f>& points, const std::shared_ptr<VoxelLocalMap>& voxel_local_map) {
    ...
    for(int iter = 0; iter < max_iteration; iter++) {
        points_world.clear();
        for(auto it: points) points_world.push_back(tmp_state.rotation * it + tmp_state.position);
